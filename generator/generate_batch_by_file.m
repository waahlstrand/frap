...
function success = generate_batch_by_file(noise_upper_bound, batch_size, directory, number_of_workers, multiple_files)
...
%         [first_moment, second_moment, variance] = moments(C_prebleach, C_postbleach, exp_sim_param);
        ...
        if multiple_files
            ...
            %x_to_save = cell2mat(x_to_save);
            %y_to_save = cell2mat(y_to_save);
...
% file_id = fopen([dir+"/x.bin"], 'w');
% fwrite(file_id, x, 'float32');
% fclose(file_id);
% 
% file_id = fopen([dir+"/y.bin"], 'w');
% fwrite(file_id, y, 'float32');
% fclose(file_id);

% file_id = fopen([dir+"/rcs.bin"], 'w');
% fwrite(file_id, first, 'float32');
% fclose(file_id);
% 
% file_id = fopen([dir+"/second.bin"], 'w');
% fwrite(file_id, second, 'float32');
% fclose(file_id);
% 
% file_id = fopen([dir+"/var.bin"], 'w');
