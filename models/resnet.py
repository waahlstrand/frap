"""Modified from pytorch models: vision/torchvision/models/resnet.py
"""

import torch
import torch.nn as nn

def conv3x3(in_planes, out_planes, stride=1, groups=1, dilation=1, dimension=2):
...
def conv1x1(in_planes, out_planes, stride=1, dimension=2):
    """1x1 convolution"""
    if dimension == 1:
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
                 base_width=64, dilation=1, norm_layer=None, dimension = 2):
        ...
        self.conv1 = conv3x3(inplanes, planes, stride, dimension=dimension)
        self.bn1 = norm_layer(planes)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = conv3x3(planes, planes, dimension=dimension)
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, in_channels=3, num_classes=1000, zero_init_residual=False,
        ...
        for m in self.modules():
            ...
            elif isinstance(m, (nn.BatchNorm2d, nn.BatchNorm1d, nn.BatchNorm3d, nn.GroupNorm)):
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilate=False, dimension=2):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                conv1x1(self.inplanes, planes * block.expansion, stride, dimension=dimension),
                norm_layer(planes * block.expansion),
            )

        layers = []
        layers.append(block(self.inplanes, planes, stride, downsample, self.groups,
                            self.base_width, previous_dilation, norm_layer, dimension=dimension))
